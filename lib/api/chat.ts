...
function dispatchStreamEvent(
  ...
  handlers: Pick<
    StreamChatParams,
    "onToken" | "onTask" | "onSource" | "onQuestion"
  >,
) {
  const data = rawEvent
    .split("\n")
...
// Wrapper non-streaming untuk chatbotwidget.tsx
export async function startNewChat(
  question: string,
): Promise<{ status: boolean; data: { chatId: string; answer: string } }> {
  let chatId = "";
  let answer = "";
  ...
  return { status: true, data: { chatId, answer } };
}

export async function continueChat(
  chatId: string,
  question: string,
): Promise<{ status: boolean; data: { answer: string } }> {
  let answer = "";
  await streamChatReply({
    question,
    chatId,
