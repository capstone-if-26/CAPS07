...
import {
  getClassifyIntentAndRelevancePrompt,
  getGenerateIntentBasedSummaryPrompt,
} from "./prompts";
import { getModuleLogger } from "@/lib/utils/logger";
import { IntentClassification, IntentRequirementsMap, OjkIntent } from "./type";
import {
  DEFAULT_TEMPERATURE,
  DEFAULT_TOP_P,
...
const SUMMARY_HEADING_TO_INTENT: Record<string, OjkIntent> = {
  "edukasi tentang hak dan pelindungan konsumen di sektor jasa keuangan, termasuk jalur pengaduan melalui APPK/Kontak OJK 157":
    "Hak Saya sebagai Konsumen",
  "panduan agar masyarakat terhindar dari investasi ilegal dan aset kripto yang tidak berizin, serta belajar mengenali entitas yang resmi diawasi OJK untuk aset keuangan digital dan kripto.":
    "Panduan Investasi & Kripto Aman",
  "materi literasi keuangan umum agar masyarakat lebih paham produk, risiko, dan pengelolaan keuangan.":
    "Literasi & Tips Keuangan",
  "cek apakah pinjaman online/fintech lending berizin atau ilegal, biasanya dengan merujuk ke daftar penyelenggara resmi OJK dan peringatan terhadap pinjol ilegal.":
    "Cek Legalitas Pinjol/Investasi",
  "edukasi tentang pola-pola scam yang sering dipakai pelaku, termasuk penipuan digital, impersonation scam, dan modus investasi/kripto ilegal.":
    "Kenali Modus Penipuan",
  "informasi dasar tentang produk perbankan seperti giro, tabungan, deposito, serta kredit/pembiayaan.":
    "Panduan Produk Bank",
  "kanal khusus untuk laporan penipuan keuangan melalui Indonesia Anti-Scam Centre (IASC).":
    "IASC — Anti-Scam Centre",
  "layanan pengaduan konsumen OJK melalui APPK/Kontak 157, termasuk telepon, WhatsApp, email, dan portal online; untuk kasus scam keuangan, laporan juga bisa diarahkan ke IASC.":
    "Lapor Penipuan (OJK / IASC)",
  "pengecekan riwayat kredit/debitur atau iDeb melalui SLIK, yang digunakan lembaga jasa keuangan untuk menilai kelayakan kredit/pembiayaan.":
...
function inferIntentFromText(text: string): OjkIntent | null {
  ...
  if (
    /\b(slik|riwayat kredit|ideb|kolektibilitas|skor kredit)\b/.test(normalized)
  ) {
    return "Cek SLIK / Riwayat Kredit";
  }

  if (
    /\b(produk bank|panduan produk bank|tabungan|giro|deposito|kredit|kpr|perbankan)\b/.test(
      normalized,
    )
  ) {
  ...
  if (
    /\b(legalitas|legal|terdaftar ojk|pinjol ilegal|investasi bodong|cek pinjol|cek investasi)\b/.test(
      normalized,
    )
  ) {
  ...
  if (
    /\b(kena tipu|ditipu|tertipu|lapor penipuan|mau lapor|pengaduan penipuan|korban penipuan)\b/.test(
      normalized,
    )
  ) {
  ...
  if (
    /\b(modus|ciri-ciri penipuan|cara kerja penipuan|menghindari penipuan|tips keamanan)\b/.test(
      normalized,
    )
  ) {
  ...
  if (
    /\b(hak konsumen|perlindungan konsumen|pelanggaran|jalur pengaduan|pengaduan konsumen)\b/.test(
      normalized,
    )
  ) {
  ...
  if (
    /\b(investasi|kripto|crypto|saham|reksa dana|risiko investasi)\b/.test(
      normalized,
    )
  ) {
  ...
  if (
    /\b(literasi|tips keuangan|edukasi keuangan|keuangan harian|menabung|anggaran)\b/.test(
      normalized,
    )
  ) {
...
export async function classifyIntentAndRelevance(
  ...
  const { systemPrompt, userPrompt } = getClassifyIntentAndRelevancePrompt(
    intentList,
    memoryText,
    question,
  );
  ...
  try {
    ...
    const isOjkRelevant = typeof rawRelevant === "boolean" ? rawRelevant : true;
...
export async function getIntentRequirements(
  intent: OjkIntent,
): Promise<string[]> {
...
function ensureRequiredSummaryPoints(
  summary: string,
...
export async function generateIntentBasedSummary(
  ...
  const requiredPointsText =
    requiredPoints.length > 0
      ? requiredPoints.map((point) => `- ${point}`).join("\n")
      : "- Ringkasan percakapan utama";

  const { systemPrompt, userPrompt } = getGenerateIntentBasedSummaryPrompt(
    intent,
    requiredPointsText,
    conversation,
  );
  ...
  try {
    ...
    const completeSummary = ensureRequiredSummaryPoints(
      summary,
      requiredPoints,
    );
