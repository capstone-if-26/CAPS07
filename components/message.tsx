...
// Feedback icons
function BotFeedback({ text, messageId }: { text: string; messageId?: string }) {
  ...
  const handleLike = async () => {
    ...
    try { await submitFeedback(messageId, newVal) } catch { setFeedback(feedback) }
  }
  ...
  return (
    <div className="flex items-center gap-2 mt-1 pl-0.5">
      <button onClick={handleCopy} title={copied ? "Disalin!" : "Salin pesan"}
        className="flex items-center justify-center hover:opacity-60 transition">
        {copied ? (
          <span style={{ color: "#999", fontSize: "11px", fontWeight: 700 }}>✓</span>
        ) : (
          <Image src="/ikon-feedback-copy.png" alt="copy" width={14} height={14} quality={100} />
        )}
      </button>

      <button onClick={handleLike} title="Respons bagus"
        className="flex items-center justify-center transition">
        <Image src="/ikon-feedback-like.png" alt="like" width={14} height={14} quality={100}
          style={{
      ...
      <button onClick={handleDislike} title="Respons kurang tepat"
        className="flex items-center justify-center transition">
        <Image src="/ikon-feedback-dislike.png" alt="dislike" width={14} height={14} quality={100}
          style={{
            filter: dislikeFlash || feedback === "dislike" ? "none" : "grayscale(0)",
...
export default function Message({ msg, index, onFlowOption }: MessageProps) {
  ...
  return (
    <div className="max-w-[85%] mx-auto flex flex-col">
      ...
      {/* Tombol source/referensi di luar bubble */}
      {msg.flow?.result?.actions && (
        <div className="grid grid-cols-1 gap-1 mt-1 w-full">
          {msg.flow.result.actions.slice(0, 1).map((action, ai) => (
            <a
              key={ai}
              href={action.url}
              target="_blank"
