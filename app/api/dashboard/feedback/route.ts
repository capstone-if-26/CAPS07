...
export async function GET(req: NextRequest) {
  ...
  const reqLog = log.child({ request_id: requestId, method: "GET", path: "/api/dashboard/feedback" });
  ...
  try {
    ...
    reqLog.info({ days, year, month, status: 200, duration: Date.now() - start }, "dashboard.feedback_fetched");
    return buildSuccessResponse(data, "Berhasil mengambil data feedback", 200);
  } catch (error: unknown) {
    reqLog.error({ err: error, status: 500, duration: Date.now() - start }, "dashboard.feedback_failed");
